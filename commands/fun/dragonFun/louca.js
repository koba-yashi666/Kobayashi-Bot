...
export default async function run(ctx){const target=targetOf(ctx);if(!target||target===ctx.sender)return ctx.reply(`• Mencione o "@" ou responda a mensagem de alguém. 🤷‍♀️\n• Exemplo: *${ctx.prefix}${ctx.command} @membro*`);const actor=tag(ctx.sender),targetTag=tag(target);const caption=`🍽️😤 ${actor} mandou ${target} lavar a louça. A guerra começou.`;return sendMedia(ctx,mediaFor(String(ctx.command||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"")),caption,[ctx.sender,target]);}
