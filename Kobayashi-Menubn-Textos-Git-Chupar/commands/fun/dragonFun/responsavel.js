...
export default async function run(ctx){const rawTarget=targetOf(ctx)||ctx.sender;const target=await displayJid(ctx,rawTarget);const value=Math.floor(Math.random()*101);const caption=traitCaption("responsavel",tag(target),value)||traitText("responsavel",tag(target),value);return sendMedia(ctx,mediaFor("responsavel"),caption,[target]);}
