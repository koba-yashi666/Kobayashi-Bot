...
export default async function run(ctx){const rawTarget=targetOf(ctx)||ctx.sender;const target=await displayJid(ctx,rawTarget);const value=Math.floor(Math.random()*101);const caption=traitCaption("misteriosa",tag(target),value)||traitText("misteriosa",tag(target),value);return sendMedia(ctx,mediaFor("misteriosa"),caption,[target]);}
