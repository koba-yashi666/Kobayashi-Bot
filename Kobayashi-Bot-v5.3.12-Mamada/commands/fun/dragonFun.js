...
const ACTIONS = ["beijo","beijar","abraco","abraço","abracar","tapa","tapar","chute","chutar","carinho","cafune","matar","louca","louça","lamber","morder","comer","gozar","mamada","socar","soco","chorao","chorona"];
...
function actionCaption(command, actor, target){
  const bank = {
    ...
    comer: `😏 ${actor} Moletou o boga do ${target} e ainda gozou dentro`,
    gozar: `😏 ${actor} gozou na cara de ${target} na próxima é na boquinha 💦 .`,
    mamada: `😏 ${actor} pediu uma mamada e  ${target} fez garganta profunda 🫢`,
...
async function sendMedia(ctx,url,caption,mentions=[]){
  ...
  try{
    ...
    await ctx.conn.sendMessage(ctx.from,isVideo?{video:{url},gifPlayback:true,caption,mentions:safeMentions}:{image:{url},caption,mentions:safeMentions},{quoted:ctx.info});
  }catch(e){
...
export default {
 ...
 async execute(ctx){
  const c=String(ctx.command||"").toLowerCase(); const n=c.normalize("NFD").replace(/[\u0300-\u036f]/g,"");
  const explicitTarget=targetOf(ctx);
  const target=explicitTarget||ctx.sender;
  ...
  if(ACTIONS.includes(n)){
    ...
    if(["comer","gozar","mamada"].includes(n)){
      try{
