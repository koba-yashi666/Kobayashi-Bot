...
export function getLevelTitle(level = 1) {
  ...
  if (lv >= 45) return "🌌 Dragão Cósmico";
  if (lv >= 40) return "👑 Dragão Imperial";
  if (lv >= 35) return "🔥 Dragão Carmesim";
  if (lv >= 30) return "🌙 Dragão Lunar";
  if (lv >= 25) return "⚡ Dragão Ascendente";
  if (lv >= 20) return "🐉 Dragão Veterano";
  if (lv >= 15) return "🐲 Jovem Dragão";
  if (lv >= 10) return "🌸 Dragão Aprendiz";
  if (lv >= 5) return "🐣 Filhote de Dragão";
  return "🥚 Dragão Iniciante";
}
...
export function trackActivity(groupJid, userJid, options = {}) {
  ...
  const data = load();
  data[groupJid] ||= {};
  const row = data[groupJid][userJid] ||= { messages: 0, lastSeen: 0, xp: 0, level: 1, lastXpAt: 0, lastXpText: "", lastXpTextAt: 0 };
  ...
  // O contador de mensagens funciona sempre, mas o XP pode ser desligado por grupo.
  // Conversa normal rende XP e comandos rendem um bônus maior, ambos com anti-farm.
  if (isLevelEnabled(groupJid) && text.length >= (isCommand ? 1 : 3)) {
    ...
    if (cooldownOk && !duplicated) {
      xpGained = isCommand ? xpGainForCommand(text) : xpGainForText(text);
...
export function getUserActivity(groupJid, userJid) {
  ...
  if (!row) return { messages: 0, lastSeen: 0, xp: 0, level: 0, title: getLevelTitle(1), ...getLevelInfoFromXp(0) };
