...
function baseDb() { return { schema: 2, groups: {}, users: {}, marriages: {}, proposals: {}, dating: {}, datingProposals: {}, sessions: {}, families: {}, clans: {}, updatedAt: Date.now() }; }
export function loadFunDb() {
  const db = readJson(DB_FILE, baseDb());
  return { ...baseDb(), ...db, groups: db.groups || {}, users: db.users || {}, marriages: db.marriages || {}, proposals: db.proposals || {}, dating: db.dating || {}, datingProposals: db.datingProposals || {}, sessions: db.sessions || {}, families: db.families || {}, clans: db.clans || {} };
}
...
export function proposeDating(from,to){
 const db=loadFunDb();
 if(from===to)return{ok:false,reason:"self"};
 if(db.marriages[from]||db.marriages[to])return{ok:false,reason:"casado"};
 if(db.dating[from]||db.dating[to])return{ok:false,reason:"namorando"};
 db.datingProposals[to]={from,at:Date.now()}; save(db); return{ok:true};
}
export function acceptDating(jid){
 const db=loadFunDb(); const p=db.datingProposals[jid];
 if(!p)return{ok:false,reason:"none"};
 if(db.marriages[jid]||db.marriages[p.from]||db.dating[jid]||db.dating[p.from]){delete db.datingProposals[jid];save(db);return{ok:false,reason:"indisponivel"};}
 const since=Date.now(); db.dating[jid]={partner:p.from,since}; db.dating[p.from]={partner:jid,since}; delete db.datingProposals[jid]; save(db); return{ok:true,partner:p.from,since};
}
export function datingOf(jid){ const d=loadFunDb().dating[jid]; return d?{partner:d.partner,since:Number(d.since||0)}:null; }
export function proposeMarriage(from,to){
 const db=loadFunDb();
 if(from===to)return{ok:false,reason:"self"};
 if(db.marriages[from]||db.marriages[to])return{ok:false,reason:"casado"};
 const a=db.dating[from], b=db.dating[to];
 if(!a||!b||a.partner!==to||b.partner!==from)return{ok:false,reason:"nao_namoram"};
 const since=Math.max(Number(a.since||0),Number(b.since||0)); const elapsed=Date.now()-since;
 if(elapsed<DAY)return{ok:false,reason:"tempo",remainingMs:DAY-elapsed};
 db.proposals[to]={from,at:Date.now()}; save(db); return{ok:true};
}
export function acceptMarriage(jid){
 const db=loadFunDb(); const p=db.proposals[jid];
 if(!p)return{ok:false,reason:"none"};
 const a=db.dating[jid], b=db.dating[p.from];
 if(!a||!b||a.partner!==p.from||b.partner!==jid){delete db.proposals[jid];save(db);return{ok:false,reason:"nao_namoram"};}
 const since=Math.max(Number(a.since||0),Number(b.since||0)); const elapsed=Date.now()-since;
 if(elapsed<DAY)return{ok:false,reason:"tempo",remainingMs:DAY-elapsed};
 db.marriages[jid]=p.from; db.marriages[p.from]=jid; delete db.proposals[jid]; delete db.dating[jid]; delete db.dating[p.from]; save(db); return{ok:true,partner:p.from};
}
...
export function simpleRpgAction(jid,action,arg=""){ const db=loadFunDb(),u=user(db,jid); const a=norm(action); let text=""; let coins=0,xp=8;
 ...
 else if(["macetar"].includes(a)){
   const wanted=norm(arg||"");
   const foods=Object.entries(u.foods||{}).filter(([,qty])=>Number(qty)>0);
   const chosen=wanted ? foods.find(([name])=>norm(name)===wanted) : foods[0];
   if(!chosen){text="🍽️ Você não tem comida pronta. Use *cozinhar* primeiro.";xp=0;}
   else {
     const [food]=chosen;
     u.foods[food]=Math.max(0,Number(u.foods[food]||0)-1);
     text=`🍜 Você macetou *${food}*. Energia recuperada!`;
     xp=10;
   }
 }
