...
export function dungeon(j,id,level=1){
 ...
 const boss=rnd(1,100)<=20,win=rnd(1,100)<=(boss?55:82),drops={};
 if(win){const rolls=boss?rnd(4,6):rnd(2,4);for(let i=0;i<rolls;i++){const m=x.drops[rnd(0,x.drops.length-1)],q=rnd(1,boss?4:(id==="abismo"?3:2));drops[m]=(drops[m]||0)+q;u.materials[m]=(u.materials[m]||0)+q}}
 save(d);return {ok:true,win,boss,x,drops,energy:u.energy,xp:win?(boss?rnd(90,150)+x.min*2:rnd(25,65)+x.min):0,coins:win?(boss?rnd(70,130)+x.min*2:rnd(15,45)+x.min):0}}
