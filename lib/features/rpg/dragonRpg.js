...
function finishVictory(player){ const battle=player.combat, enemy=battle.enemy; const awakeningWin=Boolean(battle?.awakeningBoss); const gold=randInt(enemy.gold[0],enemy.gold[1]); const levels=awardXp(player,enemy.xp); player.gold+=gold; player.rpgStats.wins++; player.rpgStats.monstersDefeated++; progressQuests(player,enemy,battle.region); let drop=null; for(const d of enemy.drops||[]){ if(Math.random()<d.chance){drop={...d,qty:1,type:'material'}; addItem(player,drop,1); break;} } if(awakeningWin){player.awakening={...(player.awakening||{}),unlocked:true,started:true,bossDefeated:true,completed:false};} player.combat=null; savePlayer(player); return {enemy,gold,xp:enemy.xp,levels,drop,player,awakeningWin}; }
