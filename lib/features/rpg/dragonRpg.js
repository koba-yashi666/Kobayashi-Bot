...
const RPG_ITEMS = {
  espada_ferro: { id:"espada_ferro", name:"Espada de Ferro", icon:"⚔️", type:"weapon", slot:"weapon", rarity:"comum", price:180, minLevel:1, bonus:{atk:5} },
  cajado_arcano: { id:"cajado_arcano", name:"Cajado Arcano", icon:"🪄", type:"weapon", slot:"weapon", rarity:"raro", price:420, minLevel:5, bonus:{mag:9,mana:10} },
  arco_elfico: { id:"arco_elfico", name:"Arco Élfico", icon:"🏹", type:"weapon", slot:"weapon", rarity:"raro", price:450, minLevel:5, bonus:{atk:6,agi:6} },
  lamina_draconica: { id:"lamina_draconica", name:"Lâmina Dracônica", icon:"🐉", type:"weapon", slot:"weapon", rarity:"epico", price:1100, minLevel:15, bonus:{atk:14,mag:5} },
  armadura_couro: { id:"armadura_couro", name:"Armadura de Couro", icon:"🥋", type:"armor", slot:"armor", rarity:"comum", price:160, minLevel:1, bonus:{hp:15,def:3} },
  armadura_orc: { id:"armadura_orc", name:"Armadura Orc", icon:"🛡️", type:"armor", slot:"armor", rarity:"raro", price:520, minLevel:7, bonus:{hp:30,def:9} },
  manto_arcano: { id:"manto_arcano", name:"Manto Arcano", icon:"🧥", type:"armor", slot:"armor", rarity:"epico", price:950, minLevel:12, bonus:{mana:25,def:6,mag:6} },
  anel_agilidade: { id:"anel_agilidade", name:"Anel da Agilidade", icon:"💍", type:"accessory", slot:"accessory", rarity:"raro", price:380, minLevel:4, bonus:{agi:7} },
  amuleto_dragon: { id:"amuleto_dragon", name:"Amuleto Dragon", icon:"📿", type:"accessory", slot:"accessory", rarity:"lendario", price:1800, minLevel:20, bonus:{hp:25,mana:20,atk:5,def:5,mag:5,agi:5} },
  pocao_pequena: { id:"pocao_pequena", name:"Poção Pequena", icon:"🧪", type:"consumivel", rarity:"comum", price:45, minLevel:1 },
  pao_aventureiro: { id:"pao_aventureiro", name:"Pão de Aventureiro", icon:"🥖", type:"consumivel", rarity:"comum", price:30, minLevel:1 },
};

const RARITY_ICON = { comum:"⚪", raro:"🔵", epico:"🟣", lendario:"🟡" };
...
function removeStats(stats, bonus) { for (const key of Object.keys(BASE_STATS)) stats[key] = Math.max(1, Number(stats[key] || 0) - Number(bonus?.[key] || 0)); }
function equipmentLabel(eq){ if(!eq)return "Nenhum"; const item=RPG_ITEMS[eq.id]||eq; return `${item.icon||"📦"} ${item.name||eq.id}`; }
function itemBonusText(item){ const b=item?.bonus||{}; const map={hp:"❤️ HP",mana:"🔷 Mana",atk:"⚔️ ATK",def:"🛡️ DEF",mag:"🔮 MAG",agi:"💨 AGI"}; return Object.entries(b).filter(([,v])=>Number(v)).map(([k,v])=>`${map[k]||k} +${v}`).join(" • ")||"Sem bônus"; }
...
export function formatRpgShop(prefix='/'){
  const rows=Object.values(RPG_ITEMS).map(i=>`┃ ${RARITY_ICON[i.rarity]||"⚪"} ${i.icon} *${i.id}* — ${i.name}
┃ 🪙 ${i.price} • Lv.${i.minLevel}${i.bonus?` • ${itemBonusText(i)}`:""}`).join("\n┣━━━━━━━━━━━━━━━━━━━━━━\n");
  return `╭━━〔 🏪 *LOJA DRAGON RPG* 〕━━╮\n${rows}\n╰━━━━━━━━━━━━━━━━━━━━━━╯\n\n🛒 Comprar: *${prefix}comprarrpg espada_ferro*\n🎒 Depois equipe com *${prefix}equipar espada_ferro*`;
}
export function buyRpgItem(jid,itemId,qty=1){
  const p=getDragonRpgPlayer(jid); if(!p)return {ok:false,reason:'missing'}; const item=RPG_ITEMS[String(itemId||'').toLowerCase()]; if(!item)return {ok:false,reason:'item'}; const n=clamp(parseInt(qty)||1,1,20); if(p.level<item.minLevel)return {ok:false,reason:'level',required:item.minLevel,current:p.level}; const total=item.price*n; if(p.gold<total)return {ok:false,reason:'gold',required:total,current:p.gold}; p.gold-=total; addItem(p,item,n); savePlayer(p); return {ok:true,item,qty:n,total,player:p};
}
export function equipRpgItem(jid,itemId){
  const p=getDragonRpgPlayer(jid); if(!p)return {ok:false,reason:'missing'}; if(p.combat)return {ok:false,reason:'combat'}; const id=String(itemId||'').toLowerCase(); const inv=p.inventory.find(x=>x.id===id&&Number(x.qty)>0); const item=RPG_ITEMS[id]; if(!inv||!item||!item.slot)return {ok:false,reason:'item'}; if(p.level<item.minLevel)return {ok:false,reason:'level',required:item.minLevel}; const slot=item.slot; const old=p.equipment?.[slot]; if(old?.id===id)return {ok:false,reason:'already'}; if(old){ const oldDef=RPG_ITEMS[old.id]||old; removeStats(p.stats,oldDef.bonus); addItem(p,oldDef,1); } inv.qty--; addStats(p.stats,item.bonus); p.equipment[slot]={id:item.id,name:item.name,icon:item.icon,rarity:item.rarity,bonus:item.bonus}; p.resources.hp=Math.min(p.stats.hp,p.resources.hp+Number(item.bonus?.hp||0)); p.resources.mana=Math.min(p.stats.mana,p.resources.mana+Number(item.bonus?.mana||0)); savePlayer(p); return {ok:true,item,old,player:p};
}
export function unequipRpgItem(jid,slotKey){
  const p=getDragonRpgPlayer(jid); if(!p)return {ok:false,reason:'missing'}; if(p.combat)return {ok:false,reason:'combat'}; const aliases={arma:'weapon',weapon:'weapon',armadura:'armor',armor:'armor',acessorio:'accessory','acessório':'accessory',accessory:'accessory'}; const slot=aliases[String(slotKey||'').toLowerCase()]; if(!slot)return {ok:false,reason:'slot'}; const old=p.equipment?.[slot]; if(!old)return {ok:false,reason:'empty'}; const item=RPG_ITEMS[old.id]||old; removeStats(p.stats,item.bonus); addItem(p,item,1); p.equipment[slot]=null; p.resources.hp=Math.min(p.resources.hp,p.stats.hp); p.resources.mana=Math.min(p.resources.mana,p.stats.mana); savePlayer(p); return {ok:true,item,player:p};
}
export function formatRpgEquipment(jid,prefix='/'){ const p=getDragonRpgPlayer(jid); if(!p)return null; return `╭━━〔 🧰 *EQUIPAMENTOS* 〕━━╮\n┃ ⚔️ Arma: *${equipmentLabel(p.equipment?.weapon)}*\n┃ 🛡️ Armadura: *${equipmentLabel(p.equipment?.armor)}*\n┃ 💍 Acessório: *${equipmentLabel(p.equipment?.accessory)}*\n╰━━━━━━━━━━━━━━━━━━━━━━╯\n\n🎒 ${prefix}rpginventario\n⚙️ ${prefix}equipar <item>\n📤 ${prefix}desequipar arma`; }
export function formatRpgSkills(jid,prefix='/'){ const p=getDragonRpgPlayer(jid); if(!p)return null; const list={escudeiro:[['Golpe de Escudo',8,1],['Fortaleza',15,6]],guerreiro:[['Corte Dracônico',9,1],['Fúria Escarlate',18,8]],mago:[['Explosão Arcana',14,1],['Meteorito Mágico',24,10]],arqueiro:[['Flecha Veloz',10,1],['Chuva de Flechas',19,8]],curandeiro:[['Luz Restauradora',12,1],['Benção Vital',20,8]]}[p.class]||[]; return `╭━━〔 ✨ *HABILIDADES* 〕━━╮\n${list.map((x,i)=>`┃ ${i+1}. *${x[0]}* — 🔷${x[1]} • Lv.${x[2]}${p.level>=x[2]?' ✅':' 🔒'}`).join('\n')}\n╰━━━━━━━━━━━━━━━━━━━━━━╯\n\nUse: *${prefix}habilidade 1* ou *${prefix}habilidade 2*`; }
export function resetRpgUsers(jids=[]){ const db=readDb(); let count=0; for(const raw of jids){ const key=normalizeUser(raw); if(key&&db.players[key]){ delete db.players[key]; count++; } } writeDb(db); return {ok:true,count}; }
export function resetRpgGlobal(){ const db=readDb(); const count=Object.keys(db.players||{}).length; writeDb({version:2,players:{}}); return {ok:true,count}; }
...
export function rpgSkill(jid, skillIndex=1){ const p=getDragonRpgPlayer(jid); if(!p)return {ok:false,reason:'missing'}; if(!p.combat)return {ok:false,reason:'no_battle'}; const skills={
  escudeiro:[{name:'Golpe de Escudo',cost:8,mag:false,mult:1.35,min:1},{name:'Fortaleza',cost:15,shield:true,min:6}],
  guerreiro:[{name:'Corte Dracônico',cost:9,mag:false,mult:1.65,min:1},{name:'Fúria Escarlate',cost:18,mag:false,mult:2.35,min:8}],
  mago:[{name:'Explosão Arcana',cost:14,mag:true,mult:1.85,min:1},{name:'Meteorito Mágico',cost:24,mag:true,mult:2.55,min:10}],
  arqueiro:[{name:'Flecha Veloz',cost:10,mag:false,mult:1.55,min:1},{name:'Chuva de Flechas',cost:19,mag:false,mult:2.15,min:8}],
  curandeiro:[{name:'Luz Restauradora',cost:12,heal:true,min:1},{name:'Benção Vital',cost:20,heal:true,strong:true,min:8}]
}; const idx=clamp(parseInt(skillIndex)||1,1,2)-1; const sk=(skills[p.class]||skills.guerreiro)[idx]; if(!sk)return {ok:false,reason:'skill'}; if(p.level<sk.min)return {ok:false,reason:'level',required:sk.min,current:p.level,skill:sk}; if(p.resources.mana<sk.cost)return {ok:false,reason:'mana',required:sk.cost,current:p.resources.mana}; p.resources.mana-=sk.cost; const e=p.combat.enemy; let heal=0,dmg=0; if(sk.shield){ p.combat.turn++; savePlayer(p); const counter=monsterTurn(p,true); return {ok:true,action:'skill',skill:sk,enemy:p.combat?.enemy,counter,player:getDragonRpgPlayer(jid)}; } if(sk.heal){ const mult=sk.strong?2.2:1.4; heal=Math.min(p.stats.hp-p.resources.hp,Math.max(sk.strong?35:18,Math.round(p.stats.mag*mult))); p.resources.hp+=heal; } else { dmg=playerDamage(p,e,sk.mag,sk.mult); e.hp=Math.max(0,e.hp-dmg); p.rpgStats.damageDealt+=dmg; } if(e.hp<=0){ savePlayer(p); return {ok:true,action:'skill',skill:sk,damage:dmg,heal,victory:finishVictory(p)}; } p.combat.turn++; savePlayer(p); const counter=monsterTurn(p,false); return {ok:true,action:'skill',skill:sk,damage:dmg,heal,enemy:e,counter,player:getDragonRpgPlayer(jid)}; }
...
export function formatDragonRpgInventory(player) {
  ...
  return `╭━━〔 🎒 *INVENTÁRIO DRAGON* 〕━━╮\n${rows.length ? rows.join("\n") : "┃ Inventário vazio."}\n┣━━━━━━━━━━━━━━━━━━━━━━\n┃ ⚔️ Arma: *${equipmentLabel(player.equipment?.weapon)}*\n┃ 🛡️ Armadura: *${equipmentLabel(player.equipment?.armor)}*\n┃ 💍 Acessório: *${equipmentLabel(player.equipment?.accessory)}*\n╰━━━━━━━━━━━━━━━━━━━━━━╯`;
}
...
export function formatRpgHelp(topic = "", prefix = "/") {
  ...
  if (["9", "equipamentos", "loja"].includes(t)) return `🧰 *EQUIPAMENTOS & LOJA*\n\nCompre em *${prefix}lojarpg* usando ouro do RPG. Equipe com *${prefix}equipar <item>* e remova com *${prefix}desequipar arma/armadura/acessorio*. Cada item possui raridade, nível mínimo e bônus próprios.`;
