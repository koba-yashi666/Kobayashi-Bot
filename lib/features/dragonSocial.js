...
export function buyShopItem(jid, itemId) {
  ...
  if (u.coins < item.price) return { ok: false, reason: "Dragon Coins insuficientes.", coins: u.coins };

  if (item.type === "title" && u.inventory[item.id]) {
    return { ok: false, reason: "Você já possui esse título.", coins: u.coins };
  }
  ...
  save(db);
  return { ok: true, item, coins: u.coins, profile: JSON.parse(JSON.stringify(u)) };
}
