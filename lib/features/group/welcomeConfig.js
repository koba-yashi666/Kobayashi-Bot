...
export function setByePhoto(groupJid, photoPath) {
  const db = readJsonFile(DB_FILE, {});
  db[groupJid] ||= {};
  db[groupJid].byePhoto = String(photoPath || "").trim();
  writeJsonFile(DB_FILE, db);
  return getWelcomeConfig(groupJid);
}

export function removeByePhoto(groupJid) {
  const db = readJsonFile(DB_FILE, {});
  if (db[groupJid]) {
    delete db[groupJid].byePhoto;
    writeJsonFile(DB_FILE, db);
  }
  return getWelcomeConfig(groupJid);
}
