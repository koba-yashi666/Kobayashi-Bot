...
const DB_PATH = path.join(process.cwd(), "\x66\x69\x6c\x65\x73", "\x64\x61\x74\x61\x62\x61\x73\x65", "\x6c\x69\x73\x74\x61\x2d\x62\x72\x61\x6e\x63\x61\x2e\x6a\x73\x6f\x6e");
...
function normalizeUser(value=""){
  ...
  if(raw.includes("@")){
    ...
    return `${digits}@${server || "\x73\x2e\x77\x68\x61\x74\x73\x61\x70\x70\x2e\x6e\x65\x74"}`;
  }
...
function sameUser(a,b){
  ...
  const aPhoneLike = !A.server || A.server === "\x73\x2e\x77\x68\x61\x74\x73\x61\x70\x70\x2e\x6e\x65\x74";
  const bPhoneLike = !B.server || B.server === "\x73\x2e\x77\x68\x61\x74\x73\x61\x70\x70\x2e\x6e\x65\x74";
...
function readDb(){
  ...
  try{
    const raw = JSON.parse(fs.readFileSync(DB_PATH, "\x75\x74\x66\x38"));

    // Formato atual.
    if(raw && typeof raw === "\x6f\x62\x6a\x65\x63\x74" && raw.groups && typeof raw.groups === "\x6f\x62\x6a\x65\x63\x74"){
      return raw;
    }

    // Migração de formato antigo: { "grupo@g.us": [jids...] }
    if(raw && typeof raw === "\x6f\x62\x6a\x65\x63\x74" && !Array.isArray(raw)){
      return {groups: raw};
    }
...
function groupEntries(db, groupJid){
  ...
  if(direct && typeof direct === "\x6f\x62\x6a\x65\x63\x74"){
